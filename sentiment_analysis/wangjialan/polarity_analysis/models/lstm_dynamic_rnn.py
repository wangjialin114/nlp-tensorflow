...
class ImdbLstm(object):
    ...
    def do_evaluation(self, sess, X, y):
        ...
        for step in range(num_steps):
            ...
            mbsz = self.batch_size
            max_seq_len = self.num_steps
            self.sequence_length = np.concatenate(
                (np.ones([10]) * max_seq_len, np.ones([10]) * (max_seq_len - (step%max_seq_len)//2)), 0)
            ...
            feed = {self.input_placeholder: input_batch, self.label_placeholder: label_batch,
                    self.initial_state_placeholder: init_state, self.sequence_length_placeholder: self.sequence_length}
    ...
    def run_epoch(self, sess, X_train, y_train, X_test, y_test):
        ...
        for epoch in range(self.max_epochs):
            ...
            for step in range(len(X_train) // self.batch_size):
                ...
                mbsz = self.batch_size
                max_seq_len = self.num_steps
                self.sequence_length = np.concatenate((np.ones([10])*max_seq_len,np.ones([10]) * ( max_seq_len - (step%max_seq_len)//2)), 0)
